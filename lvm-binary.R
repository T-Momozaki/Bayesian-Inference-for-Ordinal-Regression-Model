...
library(BayesLogit) # for sampling from Polya-Gamma
...
gs_lvm_binary = function(y, # binary response data
                         X, # predictors
                         model="probit", # selection of models, other, "t-link" and "logit" are available
  ...
  # posterior sample
  post_beta = matrix(NA, (mc-bn)/thin, p) # for beta
  ...
  w = if(is.null(w)) w=rep(1,n) else w=w
  ...
  if(model=="probit") {
    ...
    for (i in 1:mc) {
      ...
      if(i>bn & (i%%thin)==0) {post_beta[c,] = beta; c=c+1}
    }
  ...
  if(model=="t-link") {
    ...
    for (i in 1:mc) {
      ...
      if(prior=="unif"){
        beta = mvrnorm(1,
                       mu = solve(t(X)%*%diag(1/u)%*%X) %*% t(X)%*%diag(1/u)%*%z,
  ...
  if(model=="logit") {
    ...
    for (i in 1:mc) {
      # sampling z
      lb = (ifelse(y==1, 0, -Inf)-X%*%beta)/sqrt(1/w)
      ub = (ifelse(y==1, Inf, 0)-X%*%beta)/sqrt(1/w)
      z = qnorm( runif(n, pnorm(lb,0,1), pnorm(ub,0,1)), X%*%beta, sqrt(1/w) )
      
      # sampling beta
      if(prior=="unif"){
        beta = mvrnorm(1,
                       mu = solve(t(X)%*%diag(w)%*%X) %*% t(X)%*%diag(w)%*%z,
                       Sigma = solve(t(X)%*%diag(w)%*%X))
      } else if(prior=="normal") {
        beta = mvrnorm(1,
                       mu = solve(t(X)%*%diag(w)%*%X+solve(B)) %*% {t(X)%*%diag(w)%*%z+solve(B)%*%A},
                       Sigma = solve(t(X)%*%diag(w)%*%X+solve(B)))
      }
      if(i>bn & (i%%thin)==0) {post_beta[c,] = beta; c=c+1}
